...
{
    int n = max(*max_element(arr.begin(), arr.end()), *max_element(brr.begin(), brr.end()));
    vector<int> arr_frequency(n + 1), brr_frequency(n + 1);
    for (int i : arr)
        arr_frequency[i]++;
    for (int i : brr)
        brr_frequency[i]++;
    vector<int> result{};
    for (int i{}; i <= n; ++i)
        if (brr_frequency[i] - arr_frequency[i] > 0) // We only compare what in `brr` that is not in `arr`, not the other way around
            result.push_back(i);
