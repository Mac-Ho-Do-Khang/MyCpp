...
{
    size_t left{}, right{arr.size() - 1};
    int sum_left{arr[left]}, sum_right{arr[right]};
    for (; left <= right;)
    {
        ...
        else // sum_right == sum_left
        {
            if (right == left)
                return "YES";
            else // 2 sums are equal but there are still elements in between
            {
                left++;
                sum_left += arr[left];
                right--;
